...
async function getGrimoireCalendarId(auth, userId) {
  ...
  const { data } = await calendar.calendars.insert({
    requestBody: {
      summary: '💊 Pillora',
      description: "Medication schedule managed by Pillora",
      timeZone: 'UTC',
    },
  });
...
function buildEventBody(log, status = 'pending') {
  ...
  return {
    summary: `${statusPrefix}💊 ${log.elixirName} (${log.dosage})`,
    description: `Pillora medication reminder\nMedication: ${log.elixirName}\nDosage: ${log.dosage}\nScheduled: ${start.toLocaleString()}`,
